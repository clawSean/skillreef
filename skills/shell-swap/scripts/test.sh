...
make_fixture() {
  local root="$1"
  mkdir -p "$root/agents/example-agent/sessions" \
...
JSON
  ...
  # example-agent: a mix of states the rewrite must handle correctly.
  cat > "$root/agents/example-agent/sessions/sessions.json" <<'JSON'
{
  "agent:example-agent:s1": {
  ...
  "agent:example-agent:s2_diverged": {
  ...
  "agent:example-agent:s3_auto": {
    "model": "auto", "modelProvider": "nadirclaw"
  },
  "agent:example-agent:s4_fallback": {
...
ROOT="$(mktemp -d)"; make_fixture "$ROOT"
S="$ROOT/agents/example-agent/sessions/sessions.json"
run opus >/dev/null

assert_eq "s1 model switched"                "claude-opus-4-8" "$(jget "$S" "['agent:example-agent:s1']['model']")"
assert_eq "s1 provider stamped to runtime"   "claude-cli"      "$(jget "$S" "['agent:example-agent:s1']['modelProvider']")"
assert_eq "s1 source flipped (model changed)" "user"           "$(jget "$S" "['agent:example-agent:s1']['modelOverrideSource']")"
assert_eq "nested systemPromptReport.model UNTOUCHED" "gpt-5.5" "$(jget "$S" "['agent:example-agent:s1']['systemPromptReport']['model']")"
assert_eq "nested contextBudgetStatus.model UNTOUCHED" "gpt-5.5" "$(jget "$S" "['agent:example-agent:s1']['contextBudgetStatus']['model']")"
assert_eq "nested origin.provider UNTOUCHED"  "telegram"        "$(jget "$S" "['agent:example-agent:s1']['origin']['provider']")"

assert_eq "s2 diverged provider REPAIRED"     "claude-cli"      "$(jget "$S" "['agent:example-agent:s2_diverged']['modelProvider']")"
assert_eq "s2 diverged providerOverride REPAIRED" "claude-cli"  "$(jget "$S" "['agent:example-agent:s2_diverged']['providerOverride']")"

assert_eq "s3 auto model PRESERVED"           "auto"            "$(jget "$S" "['agent:example-agent:s3_auto']['model']")"
assert_eq "s3 auto provider PRESERVED"        "nadirclaw"       "$(jget "$S" "['agent:example-agent:s3_auto']['modelProvider']")"

assert_eq "s4 stale fallback origin removed"  "None"            "$(jget "$S" ".get('agent:example-agent:s4_fallback',{}).get('modelOverrideFallbackOriginProvider','None')" 2>/dev/null || echo None)"

# s5 was already exactly on target -> source must NOT be touched (provenance)
assert_eq "s5 on-target source preserved"     "user"            "$(jget "$S" "['agent:example-agent:s5_ontarget']['modelOverrideSource']")"
rm -rf "$ROOT"

# verify stale-field removal more directly
ROOT="$(mktemp -d)"; make_fixture "$ROOT"; S="$ROOT/agents/example-agent/sessions/sessions.json"
run opus >/dev/null
hasstale="$(python3 -c "import json;d=json.load(open('$S'));s=d['agent:example-agent:s4_fallback'];print(any(k.startswith('modelOverrideFallback') for k in s))")"
...
ROOT="$(mktemp -d)"; make_fixture "$ROOT"; S="$ROOT/agents/example-agent/sessions/sessions.json"
run opus >/dev/null
assert_eq "codex->claude clears agentHarnessId"        "None" "$(jget "$S" ".get('agent:example-agent:s6_codexpin',{}).get('agentHarnessId','None')")"
assert_eq "codex->claude clears agentRuntimeOverride"  "None" "$(jget "$S" ".get('agent:example-agent:s6_codexpin',{}).get('agentRuntimeOverride','None')")"
assert_eq "codex->claude clears liveModelSwitchPending" "None" "$(jget "$S" ".get('agent:example-agent:s6_codexpin',{}).get('liveModelSwitchPending','None')")"
assert_eq "codex->claude switched model"               "claude-opus-4-8" "$(jget "$S" "['agent:example-agent:s6_codexpin']['model']")"
rm -rf "$ROOT"
...
assert_eq "->codex keeps agentHarnessId"               "codex" "$(jget "$S" ".get('agent:example-agent:s6_codexpin',{}).get('agentHarnessId','None')")"
assert_eq "->codex still clears liveModelSwitchPending" "None"  "$(jget "$S" ".get('agent:example-agent:s6_codexpin',{}).get('liveModelSwitchPending','None')")"
rm -rf "$ROOT"

# ---------------------------------------------------------------------------
echo "== scoping =="
ROOT="$(mktemp -d)"; make_fixture "$ROOT"
run bogus-alias-xyz --agent example-agent --dry-run >/dev/null 2>&1
assert_exit "bogus alias rejected before agent handling" 3 "$?"
run opus --agent example-agent >/dev/null
assert_eq "scoped run leaves config primary" "openai/gpt-5.5" "$(jget "$ROOT/openclaw.json" "['agents']['defaults']['model']['primary']")"
assert_eq "scoped run switched target agent" "claude-opus-4-8" "$(jget "$ROOT/agents/example-agent/sessions/sessions.json" "['agent:example-agent:s1']['model']")"
...
ROOT="$(mktemp -d)"; make_fixture "$ROOT"
out="$(OPENCLAW_DIR="$ROOT" OPENCLAW_MCP_AGENT_ID=example-agent bash "$SWITCH" opus --agent current --dry-run 2>&1)"
assert_contains "--agent current resolves env agent" "Agent scope       : example-agent" "$out"
rm -rf "$ROOT"
...
assert_eq "cron-abort left sessions unchanged" "gpt-5.5" "$(jget "$ROOT/agents/example-agent/sessions/sessions.json" "['agent:example-agent:s1']['model']")"
rm -rf "$ROOT"
...
[[ -f "$ROOT/agents/example-agent/sessions/sessions.json.bak" ]] && ok "sessions backup created" || bad "sessions backup created"
leftover="$(find "$ROOT" -name '.shellswap-*' | wc -l | tr -d ' ')"
assert_eq "no leftover tmp files (atomic)" "0" "$leftover"
python3 -c "import json;json.load(open('$ROOT/agents/example-agent/sessions/sessions.json'))" 2>/dev/null && ok "result is valid JSON" || bad "result is valid JSON"
rm -rf "$ROOT"
...
assert_eq "abort left good store unchanged" "gpt-5.5" "$(jget "$ROOT/agents/example-agent/sessions/sessions.json" "['agent:example-agent:s1']['model']")"
rm -rf "$ROOT"

# dry-run writes nothing
ROOT="$(mktemp -d)"; make_fixture "$ROOT"
before="$(md5sum "$ROOT/openclaw.json" "$ROOT/agents/example-agent/sessions/sessions.json")"
run opus --dry-run >/dev/null
after="$(md5sum "$ROOT/openclaw.json" "$ROOT/agents/example-agent/sessions/sessions.json")"
...
assert_eq "offline think set target agent" "high" "$(jget "$S" "['agent:example-agent:s1']['thinkingLevel']")"
assert_eq "offline fast auto set" "auto" "$(jget "$S" "['agent:example-agent:s1']['fastMode']")"
...
S="$ROOT/agents/example-agent/sessions/sessions.json"
CS="$ROOT/agents/clawdia/sessions/sessions.json"
run --think default --fast default --agent example-agent --session-mode offline >/dev/null
assert_eq "default clears thinking override" "None" "$(jget "$S" ".get('agent:example-agent:s1',{}).get('thinkingLevel','None')")"
assert_eq "default clears fast override" "None" "$(jget "$S" ".get('agent:example-agent:s1',{}).get('fastMode','None')")"
...
ROOT="$(mktemp -d)"; make_fixture "$ROOT"
S="$ROOT/agents/example-agent/sessions/sessions.json"
run opus --think x-high --fast on --agent example-agent --session-mode offline >/dev/null
assert_eq "model+think normalized x-high" "xhigh" "$(jget "$S" "['agent:example-agent:s1']['thinkingLevel']")"
assert_eq "model+fast normalized on" "True" "$(jget "$S" "['agent:example-agent:s1']['fastMode']")"
assert_eq "model still switched with overrides" "claude-opus-4-8" "$(jget "$S" "['agent:example-agent:s1']['model']")"
rm -rf "$ROOT"
